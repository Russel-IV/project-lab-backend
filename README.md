...
- PostgreSQL (PostGIS on `gateway`/`inventory-service` for geo search), one database per service, Flyway migrations
...
1. Copy `.env.example` to `.env` and fill in the values (`POSTGRES_*`, `SPRING_PORT`, `SPRING_PROFILES_ACTIVE`).
