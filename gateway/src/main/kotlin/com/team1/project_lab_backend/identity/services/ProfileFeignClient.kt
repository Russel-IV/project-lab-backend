...
interface ProfileFeignClient {
    ...
    fun getProfile(@PathVariable userId: Int): ProfileResponse

    @PatchMapping("/internal/profile/{userId}")
    fun updateProfile(@PathVariable userId: Int, @RequestBody request: ProfileUpdateRequest): ProfileResponse

    @PatchMapping("/internal/profile/{userId}/picture-url")
    fun updatePictureUrl(@PathVariable userId: Int, @RequestBody request: UpdatePictureUrlRequest): ProfileResponse

    @PatchMapping("/internal/profile/{userId}/password")
    fun changePassword(@PathVariable userId: Int, @RequestBody request: PasswordChangeRequest)

    @DeleteMapping("/internal/profile/{userId}")
    fun deleteAccount(@PathVariable userId: Int)
}
