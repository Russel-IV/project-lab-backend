...
class AmenityServiceTest {
    ...
    fun createAmenityMapsFeignBadRequestWithMessage() =
        runTest {
            ...
            Mockito.`when`(amenityFeignClient.create(request)).thenThrow(webClientException(400, """{"message":"name must not be blank"}"""))

            val ex = assertThrowsSuspend<ResponseStatusException> { amenityService.createAmenity(request) }
    ...
    fun getAmenityByIdReturnsNotFoundWhenMissing() =
        runTest {
            Mockito.`when`(amenityFeignClient.get(99)).thenThrow(webClientException(404))

            val ex = assertThrowsSuspend<ResponseStatusException> { amenityService.getAmenityById(99) }
    ...
    fun updateAmenityReturnsNotFoundWhenMissing() =
        runTest {
            ...
            Mockito.`when`(amenityFeignClient.update(99, request)).thenThrow(webClientException(404))

            val ex = assertThrowsSuspend<ResponseStatusException> { amenityService.updateAmenity(99, request) }
    ...
    fun deleteAmenityReturnsNotFoundWhenMissing() =
        runTest {
            Mockito.`when`(amenityFeignClient.delete(99)).thenThrow(webClientException(404))

            val ex = assertThrowsSuspend<ResponseStatusException> { amenityService.deleteAmenity(99) }
