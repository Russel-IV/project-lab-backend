...
class ReviewResolverTest {
    ...
    fun myReviewForStayRequiresAuthentication() =
        runTest {
            val ex = assertThrowsSuspend<ResponseStatusException> { resolver.myReviewForStay(2) }
    ...
    fun myReviewsRequiresAuthentication() =
        runTest {
            val ex = assertThrowsSuspend<ResponseStatusException> { resolver.myReviews(null, null) }
            assertEquals(HttpStatus.UNAUTHORIZED, ex.statusCode)
        }
    ...
    fun createReviewPropagatesNotEligibleError() =
        runTest {
            withAuthenticatedUser(authenticatedUserId) {
                ...
                val ex =
                    assertThrowsSuspend<GraphQLBusinessException> {
                        resolver.createReview(CreateReviewInput(text = "Great stay", rating = 5, stayId = 2))
                    }
    ...
    fun deleteReviewReturnsTrueOnSuccess() =
        runTest {
            withAuthenticatedUser(authenticatedUserId) {
                Mockito.`when`(reviewService.deleteReview(eqArg(1), eqArg(1))).thenReturn(Unit)
